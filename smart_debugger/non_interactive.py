...
class NonInteractiveDebugger:
    ...
    def __init__(self, file_path: str, line_no: int, command: str, quiet_mode: bool = False):
    ...
    def trace_function(self, frame, event, arg):
            ...
            try:
                ...
                if resolved_path == self.file_path_str and lineno == self.line_no:
                    ...
                    if self.quiet_mode:
                        ...
                        real_sys.__stderr__.write(f"Breakpoint hit: {filename}:{lineno}\n")
                        
                        # Warn if no output was captured
                        if not self.repl_output.strip():
                            real_sys.__stderr__.write("Warning: No output captured from REPL command\n")
        ...
        if event == 'call':
            ...
            if any(skip in filename for skip in ['pytest', 'pluggy', '_pytest', 'importlib', 'site-packages']):
                return None  # Don't trace this file
                
        return self.trace_function


def run_non_interactive_debug(file_path: str, line_no: int, command: str, pytest_args: list, quiet_mode: bool = False) -> int:
    """
