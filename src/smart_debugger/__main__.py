...
def main():
    ...
    if mode not in ["pytest", "standalone"]:
        print(
            f"Error: Invalid mode '{mode}'. Must be 'pytest' or 'standalone'",
            file=sys.stderr,
        )
        return 1
    ...
    if len(args) < 3:
        if not quiet_mode:
            print(
                "Usage: python3 -m smart_debugger [--quiet|-q] [--mode MODE] <file> <line> <command> [-- args]",
                file=sys.stderr,
            )
            print("Modes: pytest (default), standalone", file=sys.stderr)
            print("Examples:", file=sys.stderr)
            print(
                '  python3 -m smart_debugger test.py 10 "print(x)" -- -v',
                file=sys.stderr,
            )
            print(
                '  python3 -m smart_debugger --quiet test.py 10 "print(x)" -- -v',
                file=sys.stderr,
            )
            print(
                '  python3 -m smart_debugger --mode standalone script.py 42 "print(sys.argv)" -- arg1 arg2',
                file=sys.stderr,
            )
        return 1
    ...
    if mode == "pytest":
        ...
        exit_code = run_non_interactive_debug(
            file_path, line_no, command, pytest_args, quiet_mode
        )
    else:  # standalone mode
        from .standalone import run_standalone_debug

        exit_code = run_standalone_debug(
            file_path, line_no, command, pytest_args, quiet_mode, is_module
        )
