...
def main():
    ...
    if len(args) < 3:
        print("Usage: pydebug [--mode MODE] [--quiet|-q] [-m] <file> <line> <command> -- [args]", file=sys.stderr)
    ...
    # Change to substrate root
    substrate_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    os.chdir(substrate_root)
    
    # Build the command
    cmd = [sys.executable, '-m', 'smart_debugger.smart_debugger']
