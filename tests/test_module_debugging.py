...
def test_module_debugging():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (pkg_dir / "__main__.py").write_text("""import sys
...
""")
        ...
        cmd = [
            ...
            "--mode", "standalone", "-m",
            "test_package", "5", 'print(f"data={data}")'
        ]
        
        # Set PYTHONPATH to include both smart_debugger and the temp directory
        env = os.environ.copy()
        env['PYTHONPATH'] = f"{SMART_DEBUGGER_DIR}:{tmpdir}"
        
        result = subprocess.run(
            cmd,
            cwd=str(SMART_DEBUGGER_DIR),
            capture_output=True,
            text=True,
            env=env
        )
...
""")
        ...
        cmd = [
            sys.executable, "-m", "smart_debugger",
            "--mode", "standalone", "-m",
            "argmod", "5", 'print(f"sys.argv={sys.argv}")',
            "--", "10", "20", "hello", "30"
        ]
...
def test_builtin_module():
    ...
    cmd = [
        sys.executable, "-m", "smart_debugger",
        "--mode", "standalone", "-m",
        "json.tool", "38", 'print("At json.tool main")'  # Line might vary
    ]
    
    env = os.environ.copy()
    env['PYTHONPATH'] = str(SMART_DEBUGGER_DIR)
...
def test_module_not_found():
    ...
    cmd = [
        sys.executable, "-m", "smart_debugger",
        "--mode", "standalone", "-m",
        "nonexistent_module_xyz", "10", 'print("test")'
    ]
...
def test_module_quiet_mode():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        module_file.write_text("""data = [1, 2, 3, 4, 5]
total = sum(data)
...
""")
        ...
        cmd = [
            sys.executable, "-m", "smart_debugger",
            "--mode", "standalone", "-m", "--quiet",
            "quiet_test", "3", 'print(f"data={data}, total={total}")'
        ]
